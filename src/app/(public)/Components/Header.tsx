...
export default function Header() {
  ...
  useEffect(() => {
    ...
    if (isOpen) {
      const setIsVisibleMenuToTrue = () => setIsVisible(true);
      setIsVisibleMenuToTrue();
  ...
  return (
    <header className="w-full bg-white flex items-center justify-between shadow-md fixed top-0 left-0 z-50">
      ...
      <div className="flex rounded items-center space-x-3 ml-2 sm:ml-4 md:ml-6">
        <Image src="/images/genstohubLogo.jpg" alt="prep Logo" width={70} height={70} priority />
        <div className="ml-[-25px]">
          <h1 className="text-xl font-semibold text-gray-800 px-6 pr-30">
            PREP CENTER
          </h1>
          <p className="text-xs text-gray-500 pl-6 mb-2">
      ...
      <nav className="mr-4">
        <ul className="hidden md:flex space-x-5 ml-2">
          ...
          <li>
            <Link href="/sign-up" className="text-gray-700 hover:text-sky-600">
              SignUp
            </Link>
          </li>
          <li>
      ...
      {isVisible && (
        ...
        >
          <Link
            href="/"
            onClick={closeMenu}
            className="block py-2 px-6 text-gray-800 hover:text-blue-600 font-medium"
          >
            Home »
          </Link>
          <Link
