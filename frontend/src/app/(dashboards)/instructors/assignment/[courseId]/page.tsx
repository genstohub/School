...
import { 
  ...
  UploadCloud, Link as LinkIcon, Send,
  Briefcase, BarChart, ClipboardCheck
} from "lucide-react";
...
export default function AssignmentCreationPage({ params }: { params: Promise<{ courseId: string }> }) {
  ...
  const updateField = <K extends keyof AssignmentData>(field: K, value: AssignmentData[K]) => {
  ...
  return (
    <main className="min-h-screen bg-[#040605] rounded-3xl text-white p-6 md:p-12">
      <div className="max-w-6xl mx-auto">
        ...
        {/* Navigation */}
        <Link href="/instructors" className="inline-flex items-center gap-2 text-gray-500 hover:text-emerald-500 mb-10 group transition-colors">
          <ChevronLeft size={18} className="group-hover:-translate-x-1 transition-transform" />
          <span className="text-[10px] font-black uppercase tracking-widest">Instructor Hub</span>
        </Link>
        ...
        <header className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-8 mb-16 border-l-4 border-emerald-600 pl-6">
          ...
          <div className="flex gap-4">
            <div className="bg-emerald-600/10 border border-emerald-500/20 px-6 py-3 rounded-2xl flex items-center gap-3 text-emerald-500">
              <BarChart size={18} />
              <span className="text-lg font-black">{form.weightage}% Grade Weight</span>
            </div>
          </div>
        </header>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
          ...
          <div className="lg:col-span-8 space-y-10">
            <section>
              <label className="text-[10px] font-black uppercase text-gray-500 tracking-widest mb-4 block">Project Title</label>
              <input 
                value={form.title}
            ...
            <section>
              <label className="text-[10px] font-black uppercase text-gray-500 tracking-widest mb-4 block">Detailed Instructions</label>
              <div className="bg-[#0A0C0B] border border-gray-800 rounded-[2rem] p-6 focus-within:border-emerald-600/50 transition-all">
                ...
                <div className="flex gap-4 pt-4 border-t border-gray-900">
                  <button type="button" className="text-gray-500 hover:text-white transition-colors"><LinkIcon size={18}/></button>
                  <button type="button" className="text-gray-500 hover:text-white transition-colors"><UploadCloud size={18}/></button>
                </div>
              </div>
            </section>
          </div>
          ...
          <aside className="lg:col-span-4 space-y-6">
            <div className="bg-[#0A0C0B] border border-gray-800 rounded-[2.5rem] p-8 space-y-8">
              ...
              <div>
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <span className="text-[10px] font-black uppercase tracking-widest text-gray-400">Submission Deadline</span>
                </div>
                <input 
                  type="datetime-local"
                  value={form.dueDate}
              ...
              {/* Submission Type */}
              <div>
                <span className="text-[10px] font-black uppercase tracking-widest text-gray-400 block mb-4">Submission Format</span>
                <div className="grid grid-cols-1 gap-2">
                  {(["File Upload", "External Link", "Text Entry"] as const).map((type) => (
                    <button
                      key={type}
                      type="button"
                      ...
                      className={`p-4 rounded-xl text-[10px] font-black uppercase tracking-widest border transition-all flex items-center justify-between ${
                        form.submissionType === type ? "bg-emerald-500 text-black border-emerald-500" : "bg-transparent border-gray-800 text-gray-500 hover:border-gray-700"
                      }`}
                    >
                      {type}
                      {form.submissionType === type && <ClipboardCheck size={14} />}
                    </button>
                  ))}
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <span className="text-[10px] font-black uppercase tracking-widest text-gray-400 block mb-2">Max Points</span>
                  <input 
                    type="number"
                    value={form.maxPoints}
                    onChange={(e) => updateField('maxPoints', parseInt(e.target.value) || 0)}
                ...
                <div>
                  <span className="text-[10px] font-black uppercase tracking-widest text-gray-400 block mb-2">Weight %</span>
                  <input 
                    type="number"
                    value={form.weightage}
                    onChange={(e) => updateField('weightage', parseInt(e.target.value) || 0)}
                    className="w-full bg-gray-950 border border-gray-800 p-4 rounded-xl text-sm font-black outline-none focus:border-emerald-500 transition-all"
                  />
                </div>
              </div>
            ...
            <div className="p-6 bg-amber-500/5 border border-amber-500/20 rounded-3xl flex gap-4">
              ...
              <p className="text-[9px] font-bold uppercase text-amber-500/80 leading-relaxed">
                Once deployed, changing the grade weightage may affect existing student cumulative records.
              </p>
            </div>
        ...
        {isSuccess && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center bg-gray-950/95 backdrop-blur-2xl p-6">
            ...
            >
              ...
              <h2 className="text-4xl font-black tracking-tighter uppercase mb-2">Deployed</h2>
