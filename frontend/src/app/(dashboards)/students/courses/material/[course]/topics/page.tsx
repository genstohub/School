...
export default function CourseMaterialTopicsPage() {
  ...
  const fetchTopics = async () => {
    try {
      ...
      // Target the specific course directory in your backend
      const res = await fetch(`/api/students/courses/topics?courseId=${params.course}`);
      ...
      const data: MaterialTopic[] = await res.json();
      setTopics(data);
    } catch (err: unknown) {
        ...
        <AnimatePresence mode="wait">
          {loading ? (
            ...
            >
              ...
              <p className="text-gray-500 text-sm mb-8 max-w-xs mx-auto">{error}</p>
              <button 
                onClick={fetchTopics}
            ...
            >
              <header className="mb-12">
                ...
                <h1 className="text-4xl font-black text-white mb-2">{courseCode}</h1>
                <p className="text-gray-500 text-sm">Select a specific topic to begin your study session.</p>
              </header>
              ...
              <div className="grid grid-cols-1 gap-4">
                {topics.map((topic, index) => (
                  ...
                  >
                    <Link
                      href={`/students/courses/material/${params.course}/topics/${topic.id}`}
            ...
            >
              ...
              <h3 className="text-gray-400 font-bold uppercase text-[10px] tracking-[0.2em]">Curriculum Empty</h3>
              <p className="text-gray-600 text-xs mt-2 max-w-xs mx-auto">
                No materials have been detected for this course. Please contact your instructor.
              </p>
            </motion.div>
          )}
        </AnimatePresence>
